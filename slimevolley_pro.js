...
var humanHasControlledPlayer1 = false; // Adding these player 1 and 2 variables to make it easier
var humanHasControlledPlayer2 = false; // to pass back control based on distance and alter difficulty of opposing AI
var trainer = null;
var generationCounter = 0;
var baseScoreFontSize = 64;
// **** Adding special currentFrame variable to keep track of frame
var currentFrame = 0;
...
Agent.prototype.printState = function() {
  ...
  // Adding in my own distance function since the other one won't work
  // for whatever godforsaken reason
  var dy = state.by - state.y;
  var dx = state.bx - state.x;
  var dist2 = (dx*dx+dy*dy);

  // Was thinking about including some additional check for handing over control 
  // involving the speed of the ball but it doesn't really change greatly
  // var ballVelocityX = state.bvx;
  // var ballVelocityY = state.bvy;
  // var ballMagnitude = sqrt(ballVelocityX * ballVelocityX + ballVelocityY * ballVelocityY)
  

  stateText += 'Distance from ball: ' + Math.round(dist2) + '\n';
  // stateText += 'Ball speed x: ' + Math.round(state.bvx*r)/r + '\n';
  // stateText += 'Ball speed y: ' + Math.round(state.bvy*r)/r + '\n';
  // stateText += 'Ball magnitude: ' + Math.round(ballMagnitude*r)/r + '\n';
...
function initGame() {
  ...
  human1 = false;
  human2 = false;
  humanHasControlledPlayer1 = false;
  humanHasControlledPlayer2 = false;
...
function update(nStep, currentFrame) {
  "use strict";

  // adding in current frame number to limit updates to AI
  currentFrame = currentFrame || 1;


  var result = 0;
  // Checking to see if turning off this particular section will default to AI
  // var turnOffInput = true;
  ...
  for (var step = 0; step < nStep; step++) {
    ...
    // ****
    // We're going to limit the frequency with which 
    // agent 1 gets updates. We will generalise this
    // to both agents in the future, the only issue
    // being we actually do want to the states to update
    // each frame 
    // We use prime factorisation to allow there to be patterns
    // of frames missed which leads to more interesting mistakes
    // also when it's only every 3rd or 4th frame that gets missed
    // it makes no real difference to the performance of the AI
    // **** 
    if(humanHasControlledPlayer2)
    {
      if(currentFrame % 2 == 0 || currentFrame % 3 == 0 )
      { 
        // console.log("updated state + is step divisible by 4: ", currentFrame);
...
function keyboardControl() {
  ...
  // player 1 distance
  var dy = game.agent1.loc.y - game.ball.loc.y;
  var dx = game.agent1.loc.x - game.ball.loc.x;
  var dist2_player1 = (dx*dx+dy*dy);
  ...
  // player 2 distance
  var dy = game.agent2.loc.y - game.ball.loc.y;
  var dx = game.agent2.loc.x - game.ball.loc.x;
  var dist2_player2 = (dx*dx+dy*dy);
  ...
  if (keyIsDown(a1_forward) || keyIsDown(a1_backward) || keyIsDown(a1_jump)) {
    ...
    humanHasControlledPlayer1 = true;
  }

  // if the distance is less than some value we set human control to false
  // however if the ball is not that close but humanHasControlled as some stage
  // hand back control to the human
  if(dist2_player1 < min_dist2){
    human1 = false;
    humanHasControlled = false;
  } else if (humanHasControlledPlayer1) {
    human1 = true;
  }
  ...
  if (keyIsDown(a2_forward) || keyIsDown(a2_backward) || keyIsDown(a2_jump)) {
    human2 = true;
    humanHasControlled = true;
    humanHasControlledPlayer2 = true;
  }

  // if the distance is less than some value we set human control to false
  // however if the ball is not that close but humanHasControlled as some stage
  // hand back control to the human
  if(dist2_player2 < min_dist2){
    human2 = false;
    humanHasControlled = false;
  } else if (humanHasControlledPlayer2) {
    human2 = true;
  }
...
function draw() {
  ...
  // also testing out this current frame nonsense
  currentFrame++;
  result = update(1, currentFrame);
  ...
  // printing game state regardless to get information needed to tune distance 
  // from the ball the agent needs to be
